...
Future<List<Announcement>> fetchAnnouncement() async {
  final response =
      await http.get('${_announcementApiUrl}announcement?all=true');
...
class AnnouncementsList extends StateNotifier<List<Announcement>> {
  ...
  Future<void> add(List<String> values) async {
    final _addAnnouncementUrl =
        '${_announcementApiUrl}add?title=${values[0]}&author=${values[1]}';
    try {
      await http.post(_addAnnouncementUrl);

      state = [...state, Announcement(title: values[0], author: values[1])];
    } catch (err) {
  ...
  void remove(Announcement target) {
    ...
    state = state
        .where((announcement) => announcement.title != target.title)
        .toList();
  }
}
